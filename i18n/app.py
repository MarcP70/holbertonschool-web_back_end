...
@app.route('/')
def index() -> str:
    """Return the homepage index with the current time"""
    ...
    try:
        ...
        now = datetime.now(pytz.timezone(app.config['BABEL_DEFAULT_TIMEZONE']))

    # Format the current time based on the locale
    if locale == 'fr':
        current_time = now.strftime('%d %b %Y à %H:%M:%S')
        # Custom month names for French
        month_names = {
            'Jan': 'janv.', 'Feb': 'févr.', 'Mar': 'mars', 'Apr': 'avr.', 'May': 'mai',
            'Jun': 'juin', 'Jul': 'juil.', 'Aug': 'août', 'Sep': 'sept.', 'Oct': 'oct.',
            'Nov': 'nov.', 'Dec': 'déc.'
        }
        for eng, fr in month_names.items():
            current_time = current_time.replace(eng, fr)
    else:
